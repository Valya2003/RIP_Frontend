...
import {createResistor} from "store/slices/resistorsSlice.ts";
import {T_ResistorAddData} from "modules/types.ts";

const ResistorAddPage = () => {
    ...
    const handleCreateResistor = async() => {
        ...
        formData.append('resistance', resistance as string)

        if (imgFile != undefined) {
            formData.append('image', imgFile, imgFile.name)
        }

        await dispatch(createResistor(formData as T_ResistorAddData))
        ...
        <Container>
            <Row>
                ...
                <Col md={6}>
                    ...
                    <CustomInput label="Сопротивление" placeholder="Введите сопротивление" type="number" value={resistance} setValue={setResistance}/>
                    <Col className="d-flex justify-content-center gap-5 mt-5">
                        <Button color="success" className="fs-4" onClick={handleCreateResistor}>Создать</Button>
                    </Col>
                </Col>
            </Row>
        </Container>
...
export default ResistorAddPage