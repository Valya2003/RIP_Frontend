...
const CalculationsPage = () => {
    ...
    useEffect(() => {
        handleFetchCalculations()
    }, [filters]);

    useEffect(() => {
        const intervalId = setInterval(handleFetchCalculations, 2000)
        return () => clearInterval(intervalId)
    }, [filters]);

    const handleFetchCalculations = () => dispatch(fetchCalculations())
    ...
    return (
        <Container>
            ...
            {calculations.length ? <CalculationsTable calculations={calculations} /> : <h3 className="text-center mt-5">Вычисления не найдены</h3>}
        </Container>
    )
};
